...
def execute(data_frame: pd.DataFrame, percentage_of_training_data: float):
    ...
    print(data_frame["open"])

    to_row = int(len(data_frame) * percentage_of_training_data)
    training_data = list(data_frame[0:to_row]['open'])

    # training_data = []
    # for column in data_frame.columns:
    #     training_data.append(list(data_frame[0:to_row][column]))

    training_data_exo = np.dstack([list(data_frame[0:to_row]['open']),
                                   list(data_frame[0:to_row]['close'])]
                                  )
    ...
    for i in range(n_test_obser):
        ...
        model = ARIMA(endog=training_data, order=(4, 1, 0))
        model_fit = model.fit()
